...
class TournamentView(View):
    ...
    def select_tournament(self, tournaments):
        ...
        print(
            f"{i + 1} - {tournament['name']} à {tournament['location']} du {tournament['start_date']} au "
            f"{tournament['end_date']} - Rounds: {tournament['rounds']} - Inscrits: {registered_players}")
        while True:
            try:
                ...
                if tournament_index < 0 or tournament_index >= len(tournaments):
                    ...
                    selected_tournament = tournaments_reversed[tournament_index]['id']
